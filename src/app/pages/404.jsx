...
const NotFound = () => {
	...
	// Détection du mode sombre/systeme
	useEffect(() => {
		const checkDarkMode = () => {
	...
	// Création des étoiles, animations et trou noir
	useEffect(() => {
		const moveAstro = () => {
		...
		const animateBlackHole = () => {
			...
			if (blackHole) {
				// Animation de rotation du trou noir
				blackHole.animate(
					[
		...
		>
			...
			<main className="flex flex-col md:flex-row items-center justify-center px-10 gap-12 flex-1 z-10">
				<div className="space-y-6 text-center md:text-left">
					<div>
						<h1 className="text-8xl font-bold bg-clip-text text-white">404</h1>
						<h2 className="text-4xl font-semibold mt-2">Page introuvable</h2>
					</div>
					<p className="text-gray-300 mt-3 max-w-sm">
						Désolé, la page que vous essayez de consulter est introuvable. 
						Elle a peut-être été aspirée par un trou noir.
					</p>
				...
				<div className="relative w-full max-w-md z-10">
					...
					<div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-center">
						<p className="text-gray-400 text-sm">Je me sens un peu perdu...</p>
					</div>
				</div>
			</main>
