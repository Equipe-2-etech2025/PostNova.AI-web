...
const LandingPage = ({
	 ...
	 *     }
	 *   },
	 *   created_at: "2025-08-29 13:23:51",
	 *   id: 65,
	 *   is_published: false,
	 *   updated_at: "2025-08-29 13:23:51"
	 * }
	 */

	// Initialiser le contenu depuis les données de l'API
	const initializeContent = (apiData) => {
		if (
			!apiData ||
			!apiData.content ||
			!apiData.content.template ||
			!apiData.content.template.data
		) {
			...
			return {
				...
				footer: {
					type: "footer",
					text: "© 2025 Ma Société. Tous droits réservés.",
					links: [
		...
		const templateData = apiData.content.template;

		// Transformer les données de l'API vers la structure attendue par le composant
		return {
			html: templateData.content?.html,
			colors: templateData.content?.colors || {
				primary: "#E41B17",
				secondary: "#FFFFFF",
				text: "#222222",
			},
			company: templateData.content?.company || "Ma Société",
			hero: templateData.content?.hero || {
				type: "hero",
				title: "Titre Principal",
				subtitle: "Sous-titre accrocheur",
				cta: {
			...
			sections: Array.isArray(templateData.content?.sections)
				? templateData.content.sections.map((section, index) => ({
						...section,
						id: section.id || `section-${index + 1}`,
					}))
				: [],
			footer: templateData.content?.footer || {
				type: "footer",
				text: "© 2025 Ma Société. Tous droits réservés.",
				links: [
					{ text: "Mentions légales", link: "#" },
	...
	const [content, setContent] = useState(() => initializeContent(data));
	...
	// Mettre à jour le contenu si les données de l'API changent
	// useEffect(() => {
	// 	const newContent = initializeContent(data);
	// 	setContent(newContent);
	// 	setOriginalContent(null); // Reset du contenu original
	// }, [data]);
	...
	const generateHTML = useCallback(() => {
		...
		return processTemplate(data.content.template.html, data.content.template.data) || `
<!DOCTYPE html>
<html>
<body>
<style>
    :root {
      --color-primary: ${content.colors?.primary || "#E41B17"};
      --color-secondary: ${content.colors?.secondary || "#FFFFFF"};
      --text-color: ${content.colors?.text || "#222222"};
  ...
  <section class="hero" data-id="hero">
    <h1>${content.hero?.title || "Titre Principal"}</h1>
    <p>${content.hero?.subtitle || "Sous-titre accrocheur"}</p>
    <button><a href="https://${content.hero?.cta?.link || "www.example.com"}" target="_blank">${content.hero?.cta?.text || "Découvrir"}</a></button>
  </section>

  ${(content.sections || []).map((section) => renderSection(section)).join("")}

  <footer data-id="footer">
    <p>${content.footer?.text || "© 2025 Ma Société. Tous droits réservés."}</p>
    <div class="footer-links">
      ${(content.footer?.links || [])
							.map(
								(link) => `<a href="${link.link || "#"}">${link.text || "Lien"}</a>`
							)
							.join("")}
    </div>
  </footer>
...
`;
	...
	const removeSection = useCallback(
		(sectionIndex) => {
			setContent((prev) => {
				...
				// Remove section
				updated.sections = sections.filter((_, index) => index !== sectionIndex);
	...
	const handleSave = useCallback(async () => {
		if (!data?.id) {
			console.error("Impossible de sauvegarder : ID de la landing page manquant");
			return;
		}
		...
		try {
			...
			const updateData = {
				content: {
					template: {
						html: "", // Le HTML sera régénéré côté serveur si nécessaire
						data: content,
					},
				},
			};

			const response = await updateLandingPage(data.id, updateData);

			if (response.success) {
				setOriginalContent(JSON.parse(JSON.stringify(content)));
				onUpdate(response.data);
				console.log("Landing page sauvegardée avec succès");
			} else {
	...
	const handleDelete = useCallback(async () => {
		if (!data?.id) {
			console.error("Impossible de supprimer : ID de la landing page manquant");
			return;
		}

		if (
			!window.confirm("Êtes-vous sûr de vouloir supprimer cette landing page ?")
		) {
			return;
		}
		...
		try {
			...
			const response = await deleteLandingPage(data.id);

			if (response.success) {
				onClose();
				console.log("Landing page supprimée avec succès");
			} else {
	...
	const updateHTMLContent = useCallback(
		(doc) => {
			...
			if (heroTitle)
				heroTitle.textContent = content.hero?.title || "Titre Principal";
			if (heroSubtitle)
				heroSubtitle.textContent =
					content.hero?.subtitle || "Sous-titre accrocheur";
			if (heroButton) {
				heroButton.textContent = content.hero?.cta?.text || "Découvrir";
				heroButton.href = `https://${content.hero?.cta?.link || "www.example.com"}`;
			}
			...
			if (doc.documentElement) {
				doc.documentElement.style.setProperty(
					"--color-primary",
					content.colors?.primary || "#E41B17"
				);
				doc.documentElement.style.setProperty(
					"--color-secondary",
					content.colors?.secondary || "#FFFFFF"
				);
				doc.documentElement.style.setProperty(
					"--text-color",
					content.colors?.text || "#222222"
				);
			}
