...
const ButtonOutline = ({children, circle = false, color = "default"}) => {
    ...
    return (
        <button className={`${colorClasses[color] || colorClasses['blue']} border font-bold ${circle ? "rounded-full p-2" : "rounded py-2 px-4"} hover:bg-[var(--color-lightgray)] hover:text-[var(--color-blue)] hover:border-[var(--color-lightgray)] transition duration-200 cursor-pointer`}>
            {children}
        </button>
  )
}
