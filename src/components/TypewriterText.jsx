...
const SimpleTypewriter = ({ 
  ...
  }, [displayedText, isDeleting, text, typingSpeed, deleteSpeed, pauseTime, loop]);

  const formatText = (text) => {
    // Diviser le texte en segments avec leurs couleurs
    const segments = [
      { text: "Une ", color: "text-white" },
      { text: "idée", color: "text-[#4335C4]", glow: true },
      { text: ", ", color: "text-white" },
      { text: "une ", color: "text-white", breakLine: true },
      { text: "campagne", color: "text-[#4335C4]", glow: true }
    ];
    ...
    segments.forEach((segment, segmentIndex) => {
      ...
      const visibleLength = Math.max(0, Math.min(segment.text.length, displayedText.length - segmentStart));
      ...
      if (visibleText.length > 0) {
        ...
        result.push(
          <span
            ...
            style={{
              textShadow: segment.glow ? '0 0 10px currentColor, 0 0 20px currentColor' : 'none'
            }}
          >
            {visibleText}
          </span>
      ...
      <h2 className="text-white text-xl md:text-4xl font-semibold leading-relaxed text-center">
        ...
        <span
          ...
          style={{ 
            ...
            boxShadow: showCursor ? `0 0 10px ${isDeleting ? '#4335C4' : '#4335C4'}` : 'none'
          }}
        />
      </h2>
    </div>
